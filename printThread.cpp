...
bool PrintThread::checkComplete(wxString& dataStr, const u_char * pkt_data, int size, int tcpHeaderOffset) {
	...
	// see if this is a completion to a previously started item; if so append data
	bool found = false;
	...
	//is this new? 
	bool isNewHTTP = dataStr.StartsWith(_T("GET")) || dataStr.StartsWith(_T("POST")) || dataStr.StartsWith(_T("HEAD"));
	//is it complete? (The \r\n\r\n check is more reliable than PSH flag)
	//oh and we stop with the end of the headers in a POST, we don't continue through all 
	//the other data, because if they're uploading a gigabyte to youtube, we just want the cookie
	bool end = dataStr.Contains(_T("\r\n\r\n"));
	// if new partial HTTP request, add to vector
	if (!found && !end && isNewHTTP) { 
	...
	return end && (found || isNewHTTP); //return true if complete get or completion of a previous get.
}
...
wxThread::ExitCode PrintThread::Entry() {
	...
	if (interf->description && strcmp(interf->description,"file") == 0){
		if((adhandle = pcap_open_offline(interf->name, NULL)) == NULL)
			return  (wxThread::ExitCode)-1;
	}else if ((adhandle = pcap_open_live(interf->name, 65536, PCAP_OPENFLAG_PROMISCUOUS, 1000, NULL)) == NULL){
		return  (wxThread::ExitCode)-1;
	}
