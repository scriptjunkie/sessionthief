...
wxString SystemInterface::getLocalMac(u_char* destMac) {
	...
	while(pAdapterInfo != NULL){
		wxString adapterName=wxString::FromAscii(pAdapterInfo->AdapterName);
		wxString pcapName=wxString::FromAscii(alldevs->name);
...
pcap_if_t* SystemInterface::getInterface() {
	...
	wxString errMessage(_T("No interface selected. Open pcap file."));
	if (pcap_findalldevs(&firstdev, errbuf) == -1 || firstdev == NULL)
		errMessage = _T("No interfaces. Open pcap file.");
	...
	//Get a filename ready if they want to open a file
	...
	} else { //looks like the user didn't want to do anything or there were no interfaces
		tcpdumpPid = 0;
#ifndef WIN32
		if (devs.size() == 0){
			//No interfaces. Maybe tcpdump will work. "sudo chmod 4755 $(which tcpdump)" makes tcpdump setuid
			wxArrayString tcpdumpDevs;
			wxExecute(_T("tcpdump -D"), tcpdumpDevs);
			if(tcpdumpDevs.Count() != 0){ //if we found interfaces
				...
				if (dialog.ShowModal() == wxID_OK){
					...
					wxString command(_T("sh -c 'tcpdump -U -w - -n -i "));
					command << (dialog.GetSelection() + 1) << _T(" > ") << pipeName << _T("'");
					tcpdumpPid = wxExecute(command,wxEXEC_MAKE_GROUP_LEADER | wxEXEC_ASYNC);
					wxFileName pipeObj(pipeName);
					while(pipeObj.GetSize() < 1)
						wxMilliSleep(100); // wait for tcpdump init
					fileName = pipeName;
				}
			}
		}
#endif
		if(fileName == _T("")){
			wxFileDialog filediag(NULL, errMessage);
			if(filediag.ShowModal() == wxID_CANCEL)
				exit(1);
			fileName = filediag.GetPath();
		}
		alldevs = (pcap_if_t*)calloc(1,sizeof(pcap_if_t));
		alldevs->name = strdup(fileName.ToAscii());
		alldevs->description = strdup("file");
	}
	delete [] choices;
...
long SystemInterface::tcpdumpPid;
