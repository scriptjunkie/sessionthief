...
#pragma implementation "nviewFrame.cpp"
#endif
...
void nviewFrame::OnAprMenuClick(wxCommandEvent& WXUNUSED(event)) {
	...
	if(data == 0)
		if(wxMessageBox(wxString::FromAscii("IP forwarding not enabled! Continue?\n"
				"\nTo enable IP forwarding, run this and reboot:\n"
    ...
    if (!aprRunning) {
        aprMenu->FindItemByPosition(1)->SetText(_T("APR"));
        return;
    }
    aprMenu->FindItemByPosition(1)->SetText(_T("Stop APR"));    try{
        ArpThread::startAprThread(ipBox->GetValue(), interf, textBox, &aprRunning);
...
void nviewFrame::OnAboutMenuClick(wxCommandEvent& WXUNUSED(event)){
    ...
    info.SetCopyright(_T("(C) 2009 scriptjunkie <scriptjunkie.1@gmail.com>"));
...
void nviewFrame::OnProfileMenuClick(wxCommandEvent& WXUNUSED(event)){
	CookieEater::ffoxArgs = wxString::FromAscii("");
	...
	if(!prefs.empty()){
		...
		CookieEater::ffoxArgs = wxString::FromAscii(" -profile \"") + CookieEater::settingsFolder;
#ifdef WIN32
		SetEnvironmentVariableA("MOZ_NO_REMOTE","1"); //screw firefox
#endif
	}
}
